        ...
        || window.location.pathname == "/messagerie/nouvelle-discussion/") { 
            ...
            if(navigator.geolocation) {
                ...
                function onSuccess(geolocation_pos) {
                    ...
                    geocode_promise.then(
                        ...
                        function() {  // Promise rejected
                            ...
                            // console.log('Erreur, on utilise le code postal associé au compte : '+jQuery('main').attr('data-basescope'));
                            scope = [];
                            more = [];
                            even_more = [];
                            // We get the other zip code of the city and the department code
                            jQuery.getJSON("https://geo.api.gouv.fr/communes?codePostal="+jQuery('main').attr('data-basescope')+"&fields=nom,code,codesPostaux,codeDepartement,codeRegion,population&format=json&geometry=centre").then(function(commune_value) {
                                    for(var i = 0; i < commune_value[0].codesPostaux.length; i++) {
                                        scope.push(commune_value[0].codesPostaux[i]);
                                    }
                                    more = commune_value[0].codeDepartement;
                ...
                function onError() {
                    ...
                    // console.log('On utilise le code postal associé au compte : '+jQuery('main').attr('data-basescope'));
                    scope = [];
                    more = [];
                    even_more = [];
                    // We get the other zip code of the city and the department code
                ...
                // We use the registered zip code of the current user                
                // console.log('Le navigateur ne supporte pas le service de géolocalisation, on utilise le code postal associé au compte : '+jQuery('main').attr('data-basescope'));
                scope = [];
                more = [];
                even_more = [];
                // We get the other zip code of the city and the department code
        ...
        jQuery('#scope-modal #geolocalisation').click(function(e){
            ...
            */
            if(navigator.geolocation) {
                ...
                more = [];
                even_more = [];
                jQuery.getJSON("https://geo.api.gouv.fr/communes?codePostal="+jQuery('main').attr('data-basescope')+"&fields=nom,code,codesPostaux,codeDepartement,codeRegion,population&format=json&geometry=centre").then(function(dpt_value) {
                    for(var i = 0; i < dpt_value[0].codesPostaux.length; i++) {
                        scope.push(dpt_value[0].codesPostaux[i]);
                    }
                    more = dpt_value[0].codeDepartement;
