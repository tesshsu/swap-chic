...
jQuery(document).ready(function() {
    ...
    jQuery(window).bind("beforeunload", function() { 
        setInactiveUser();
    })		jQuery('.loadmore').click(function(){ 		var button = jQuery(this),		    data = {			'action': 'ajaxloadmore',			'query': swapchic_ajax.ajax_url.posts,			'page' : swapchic_ajax.ajax_url.current_page		};        		jQuery.ajax({			url : swapchic_ajax.ajax_url.ajax_url, 			data : data,			type : 'POST',			beforeSend : function ( xhr ) {				button.text('Loading...');				button.after(data);			},			success : function( response ){				if( response ) {					button.text( 'Plus articles' ).prev().after(data); // insert new posts					 swapchic_ajax.ajax_url.current_page++;					 alert(response.url);					 console.log(data.page);					 					if ( swapchic_ajax.ajax_url.current_page == swapchic_ajax.ajax_url.max_page ) 						button.remove(); // if last page, remove the button				} else {					button.remove(); // if no data, remove the button as well				}			},			error :function ( data ) {				button.text( data );			}		});	});
});
=======
