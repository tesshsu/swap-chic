...
jQuery(document).ready(function() {
    ...
    jQuery(window).bind("beforeunload", function() { 
        setInactiveUser();
    })		jQuery('.loadmore').click(function(){ 		var button = jQuery(this),		    data = {			'action': 'loadmore',			'query': swapchic_ajax.posts, // that's how we get params from wp_localize_script() function			'page' : swapchic_ajax.current_page		}; 		jQuery.ajax({			url : swapchic_ajax.ajax_url, // AJAX handler			data : data,			type : 'POST',			beforeSend : function ( xhr ) {				button.text('Loading...'); // change the button text, you can also add a preloader image			},			success : function( data ){				if( data ) { 					button.text( 'Plus articles' ).prev().after(data); // insert new posts					swapchic_ajax.current_page++; 					if ( swapchic_ajax.current_page == swapchic_ajax.max_page ) 						button.remove(); // if last page, remove the button				} else {					button.remove(); // if no data, remove the button as well				}			}		});	});
});