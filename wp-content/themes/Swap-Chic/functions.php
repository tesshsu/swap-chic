...
function getScopeLatLng($scope) {
	...
	if($url->status == 'OK') {
		...
		echo "Désolée, nous n’avons pas pu détecter votre localisation. Vérifiez que votre géolocalisation est bien activée. Merci.";
	}
}
	...
	if($postlist["even_more"] && !empty($postlist["even_more"]) && $postlist["even_more"] != null){
		...
		foreach($postlist["even_more"] as $key => $post) {
			if($post[0] == 'comments') {
				...
				get_template_part( 'partials/content/content', 'map' );
				print '<div class="alert-notice postionBas"> <a href="/ajouter-produit">Maintenant que tu fais partie de la communauté #ActGreenBeChic, commence ton swap en contactant les membres de ta ville, alors, prête ? </a> </div>';
			}
		}
	}
}
